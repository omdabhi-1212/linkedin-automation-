...
| F12 | Permission slip ("I don't know who needs to hear this") | Needs adaptation | Banned only in its stock form — the generic, front-loaded reassurance used as a scroll-stopping hook, unattached to anything specific. Fine, softened, when it's a genuine address to an actual audience or moment, placed naturally within the post rather than as an opening mystery-hook, and never using the stock phrase itself: e.g. "If you came from pure science and the business side still feels like a different language — you're not behind." Specific and grounded, not manufacturing curiosity before any content shows up. |
...
**Reading this table:** 6 of 16 formulas are banned outright — not because they "don't sound like
...
already how Om naturally writes and don't need "adapting" so much as recognizing. The remaining 8
range from usable-with-real-changes to situational-at-best — none of them get used as delivered.

**A verdict isn't permanent.** Two formulas have already moved off Banned once the actual
mechanism was examined closely enough to separate a manipulative version from an authentic one:

- **F13 (bait-and-switch):** manipulative = a staged crisis with a flagged "just kidding" reveal.
  Authentic = a real, proportionate hardship followed by a genuine turn, no reveal needed.
- **F12 (permission slip):** manipulative = the stock "I don't know who needs to hear this"
  phrase, front-loaded purely to hook. Authentic = a genuine, specific address to a real audience
  or moment, placed naturally in the post, never using the stock phrase.

Both moves followed the same pattern: the original Banned verdict was catching two different
things under one name. Revisit a verdict the same way if it turns out to be doing that — the fix
is almost always a sharper distinction, not a blanket loosening. It's also not a rubber stamp:
most of the remaining Banned formulas (comment-gate, curiosity-gap, paid-vs-free reversal,
identity-reframe guru-voice) don't have an obvious authentic version, because the manipulation
isn't incidental to them, it's the whole mechanism.
